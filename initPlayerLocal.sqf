...
// nearChemlights
/* Determines total nearby chemlights. Determines if there are any nearby chemlights. IR chemlights are excluded as it will be assumed (faslsely at times) that enemies are unable to see IR light.*/
nearChemlights = {
	params ["_player"];
...
//adjustCamo
/* Adjusts the visibility of units to the AI. NOTE that the server should be running a script to set the player "currentLighting" to the value the server gets for getLighting at the player location*/
adjustCamo = {
  ...
  _scriptHandle = [_unit] spawn {
    ...
    while {true} do {
      ...
      if (_lightOn) then{
        ...
        _lighting = _lighting + 5;  //This applies an addition  visibility penalty
      }
      else {
